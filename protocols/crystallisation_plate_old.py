...
#specify protein location on the deck:  [Slot, Well]
#As a standard it will be in the aluminium 96-well block in a PCR tube
protein_location = ["7","A1"]
...
condition_asp_height = 30
reservoir_asp_height = 1
protein_asp_height = 1
drop_disp_height = 1
...
## Also Specify whether tips need to be changed between the two drops
two_drops = False
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    condition_block = protocol.load_labware("abgene_96_wellplate_1200ul",'5')
    tiprack1 = protocol.load_labware("opentrons_96_tiprack_10ul",'1')
    tiprack2 = protocol.load_labware("opentrons_96_tiprack_10ul",'2')
    tiprack3 = protocol.load_labware("opentrons_96_tiprack_300ul",'6')

    ##Load pipettes
    p20 = protocol.load_instrument('p20_single_gen2',"right",tip_racks=[tiprack1,tiprack2])
    
    ##Transfer conditions to crystallisation plate
    
    # if max_row == 8:
    #     p50 = protocol.load_instrument('p50_multi',"left",tip_racks=[tiprack3])
    ...
    #         p50.pick_up_tip(tiprack3.wells_by_name()[tipwell])
    #         p50.transfer(res_vol, condition_block.columns_by_name()[str(tipcolumn)],
    #                      cryst_plate.columns_by_name()[str(tipcolumn + (tipcolumn-1))], new_tip ='never')   
    #         p50.drop_tip()
    ...
    if two_drops == True:
        p20.distribute(prot_drop2,prot_block.wells_by_name()[protein_location[1]],
                       [cryst_plate.wells_by_name()[well_name] for well_name in well_two_destination])
    ...
    if two_drops == True:
        p20.transfer(res_drop1,[cryst_plate.wells_by_name()[well_name] for well_name in res_destination],
                 [cryst_plate.wells_by_name()[well_name] for well_name in well_two_destination],new_tip='always')
