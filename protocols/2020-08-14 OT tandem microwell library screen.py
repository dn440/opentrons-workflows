...
def run(protocol: protocol_api.ProtocolContext):
    ...
    wells_mblue = ["A1", "B1", "A2", "B2"]
    cols_compounds = [6, 7, 8, 9, 10] # library columns to aspirate compounds from
    cols_woSAM = [3, 4, 5, 6, 7]
    cols_wSAM = [8, 9, 10, 11, 12] # (30 uM SAM final c)
    ...
    # METHYLENE BLUE TEST OF PIPETTING ACCURACY (goes first to minimize the time when CBS makes contact with substrate)
    p10.distribute(2, sample_plate.columns_by_name()["1"], [reaction_plate.wells_by_name()[i] for i in wells_mblue], new_tip = 'once') # 1 mM Methylene Blue
    p50.distribute(18, sample_plate.columns_by_name()["2"], [reaction_plate.wells_by_name()[i] for i in wells_mblue], new_tip = 'once') # MilliQ. Later, measure the absorbance in this column to rule out cross-contamination.
    ...
    for i in range(0, len(cols_compounds)): # iterate through every column on the compound plate
        ...
        wells_target = [wells_reaction_woSAM[i], wells_reaction_wSAM[i]] # distant wells to distribute to (each compound is tested w/ and w/o SAM)
    ...
    p50.distribute(10, sample_plate.columns_by_name()["3"], [reaction_plate.wells_by_name()[i] for i in wells_reaction], new_tip = 'once')
    
    # Transfer 8 uL reaction mix to reaction plate
    p50.distribute(8, sample_plate.columns_by_name()["4"], [reaction_plate.wells_by_name()[i] for i in wells_reaction_woSAM], new_tip = 'once') # w/o SAM
    p50.distribute(8, sample_plate.columns_by_name()["5"], [reaction_plate.wells_by_name()[i] for i in wells_reaction_wSAM], new_tip = 'once') # w/ SAM (30uM)
